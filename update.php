...
function applyDatabaseSchemaChanges($db) {
    ...
    $requiredTables = [
        ...
        // Drag-and-Drop Email Builder tables
        ...
        'media_library' => "CREATE TABLE IF NOT EXISTS media_library (
            id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        'integration_connections' => "CREATE TABLE IF NOT EXISTS integration_connections (
            ...
            user_id INT NOT NULL,
            name VARCHAR(100) NOT NULL,
            auth_type ENUM('oauth', 'api_key', 'credentials', 'webhook', 'custom') NOT NULL,
        ...
        'integration_logs' => "CREATE TABLE IF NOT EXISTS integration_logs (
            id INT AUTO_INCREMENT PRIMARY KEY,
            integration_id INT NOT NULL,
...
// Add after line 715 (after the AI features check and before the 'features' table check)

// Check for Email Builder tables and create them if they don't exist
$checkEmailTemplates = $db->query("SHOW TABLES LIKE 'email_templates'");
if ($checkEmailTemplates->num_rows === 0) {
    // Create email templates table
    $db->query($requiredTables['email_templates']);
    
    // Insert default templates
    $defaultTemplate = '<!DOCTYPE html><html><head><title>Basic Template</title></head><body><div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif;"><div style="background-color: #f5f5f5; padding: 20px; text-align: center;"><h1>{{title}}</h1></div><div style="padding: 20px;">{{content}}</div><div style="background-color: #f5f5f5; padding: 10px; text-align: center; font-size: 12px;"><p>© ' . date('Y') . ' Your Company. All rights reserved.</p><p>You received this email because you signed up for our newsletter.</p><p><a href="{{unsubscribe_link}}">Unsubscribe</a></p></div></div></body></html>';
    
    $stmt = $db->prepare("INSERT INTO email_templates (name, description, content, is_system) VALUES (?, ?, ?, 1)");
    $templateName = 'Basic Template';
    $templateDesc = 'A simple, responsive email template with header, body, and footer sections.';
    $stmt->bind_param("sss", $templateName, $templateDesc, $defaultTemplate);
    $stmt->execute();
    
    $dbUpdates[] = "Created Email Builder template tables with default template";
}

$checkTemplateComponents = $db->query("SHOW TABLES LIKE 'template_components'");
if ($checkTemplateComponents->num_rows === 0) {
    // Create template components table
    $db->query($requiredTables['template_components']);
    
    // Insert some default components
    $defaultComponents = [
        [
            'name' => 'Header with Logo',
            'description' => 'A header section with logo and title',
            'category' => 'header',
            'html_content' => '<div style="background-color: #f5f5f5; padding: 20px; text-align: center;"><img src="{{logo_url}}" alt="Logo" style="max-height: 60px;"><h1>{{title}}</h1></div>',
            'icon' => 'fa-heading'
        ],
        [
            'name' => 'Text Block',
            'description' => 'Simple text paragraph block',
            'category' => 'content',
            'html_content' => '<div style="padding: 15px;"><p>{{text}}</p></div>',
            'icon' => 'fa-paragraph'
        ],
        [
            'name' => 'Image Block',
            ...
            'category' => 'content',
            'html_content' => '<div style="padding: 15px; text-align: center;"><img src="{{image_url}}" alt="{{alt_text}}" style="max-width: 100%;"><p style="font-size: 12px; color: #666;">{{caption}}</p></div>',
            'icon' => 'fa-image'
        ],
        [
            'name' => 'Button',
            ...
            'category' => 'content',
            'html_content' => '<div style="padding: 15px; text-align: center;"><a href="{{button_url}}" style="background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px; display: inline-block;">{{button_text}}</a></div>',
            'icon' => 'fa-mouse-pointer'
        ],
        [
            'name' => 'Footer',
            'description' => 'Standard footer with unsubscribe link',
            'category' => 'footer',
            'html_content' => '<div style="background-color: #f5f5f5; padding: 10px; text-align: center; font-size: 12px;"><p>© ' . date('Y') . ' {{company_name}}. All rights reserved.</p><p>You received this email because you signed up for our newsletter.</p><p><a href="{{unsubscribe_link}}">Unsubscribe</a></p></div>',
            'icon' => 'fa-copyright'
        ]
    ];
    
    $stmt = $db->prepare("INSERT INTO template_components (name, description, category, html_content, icon, is_system) VALUES (?, ?, ?, ?, ?, 1)");
    
    foreach ($defaultComponents as $component) {
        $stmt->bind_param("sssss", $component['name'], $component['description'], $component['category'], $component['html_content'], $component['icon']);
        $stmt->execute();
    }
    
    $dbUpdates[] = "Created template components table with default components";
}

$checkTemplateLayouts = $db->query("SHOW TABLES LIKE 'template_layouts'");
if ($checkTemplateLayouts->num_rows === 0) {
    // Create template layouts table
    $db->query($requiredTables['template_layouts']);
    ...
    $defaultLayouts = [
        [
            ...
            'columns' => 1,
            'html_structure' => '<div class="email-container" style="max-width: 600px; margin: 0 auto;"><div class="content-block" data-id="content1"></div></div>'
        ],
        [
            'name' => 'Two Columns',
            'description' => 'Two-column layout for side-by-side content',
            'columns' => 2,
            'html_structure' => '<div class="email-container" style="max-width: 600px; margin: 0 auto;"><div style="display: inline-block; width: 48%;"><div class="content-block" data-id="left_column"></div></div><div style="display: inline-block; width: 48%; margin-left: 4%;"><div class="content-block" data-id="right_column"></div></div></div>'
        ],
        [
            'name' => 'Header + Content + Footer',
            'description' => 'Standard email layout with header, body and footer',
            'columns' => 1,
            'html_structure' => '<div class="email-container" style="max-width: 600px; margin: 0 auto;"><div class="header-block" data-id="header"></div><div class="content-block" data-id="content"></div><div class="footer-block" data-id="footer"></div></div>'
        ]
    ];
    
    $stmt = $db->prepare("INSERT INTO template_layouts (name, description, columns, html_structure, is_system) VALUES (?, ?, ?, ?, 1)");
    
    foreach ($defaultLayouts as $layout) {
        $stmt->bind_param("ssis", $layout['name'], $layout['description'], $layout['columns'], $layout['html_structure']);
        $stmt->execute();
    }
    
    $dbUpdates[] = "Created template layouts table with default layouts";
}

$checkMediaLibrary = $db->query("SHOW TABLES LIKE 'media_library'");
if ($checkMediaLibrary->num_rows === 0) {
    // Create media library table
    $db->query($requiredTables['media_library']);
    $dbUpdates[] = "Created media library table for email builder";
}

// Check for Integration Marketplace tables and create them if they don't exist
$checkIntegrations = $db->query("SHOW TABLES LIKE 'integrations'");
if ($checkIntegrations->num_rows === 0) {
    // Create integrations table
    $db->query($requiredTables['integrations']);
    ...
    $defaultIntegrations = [
        [
            'name' => 'WordPress',
            'slug' => 'wordpress',
            'description' => 'Connect your WordPress site to automatically import subscribers from contact forms.',
            'category' => 'cms',
            'version' => '1.0.0',
        ...
        [
            'name' => 'Zapier',
            'slug' => 'zapier',
            'description' => 'Connect with 3000+ apps via Zapier integration.',
            'category' => 'automation',
            'version' => '1.0.0',
            'author' => 'Lumi Solutions',
            'is_active' => 1
        ...
        [
            'name' => 'Shopify',
            'slug' => 'shopify',
            'description' => 'Connect your Shopify store to automatically sync customers and send product recommendations.',
            'category' => 'ecommerce',
            'version' => '1.0.0',
            'author' => 'Lumi Solutions',
            'is_active' => 1
    ...
    $stmt = $db->prepare("INSERT INTO integrations (name, slug, description, category, version, author, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    $dbUpdates[] = "Created integrations table with default integrations";
}

$checkIntegrationConnections = $db->query("SHOW TABLES LIKE 'integration_connections'");
if ($checkIntegrationConnections->num_rows === 0) {
    // Create integration connections table
    $db->query($requiredTables['integration_connections']);
    $dbUpdates[] = "Created integration connections table";
}

$checkWebhooks = $db->query("SHOW TABLES LIKE 'webhooks'");
if ($checkWebhooks->num_rows === 0) {
    // Create webhooks table
    $db->query($requiredTables['webhooks']);
    $dbUpdates[] = "Created webhooks table";
}

$checkIntegrationLogs = $db->query("SHOW TABLES LIKE 'integration_logs'");
if ($checkIntegrationLogs->num_rows === 0) {
    // Create integration logs table
    $db->query($requiredTables['integration_logs']);
    $dbUpdates[] = "Created integration logs table";
}

// Update the features table with new features
if ($checkFeatures->num_rows > 0) {
    // Check if email_builder feature exists
    $checkEmailBuilderFeature = $db->query("SELECT COUNT(*) as count FROM features WHERE feature_name = 'email_builder'");
    $row = $checkEmailBuilderFeature->fetch_assoc();
    
    if ($row['count'] == 0) {
        // Add email_builder feature
        $db->query("INSERT INTO features (feature_name, enabled, added_version, description) 
                    VALUES ('email_builder', 1, '$latestVersion', 'Enhanced drag-and-drop email builder with templates, components and responsive layouts')");
        $dbUpdates[] = "Added Email Builder feature to features table";
    }
    
    // Check if integrations_marketplace feature exists
    $checkIntegrationsFeature = $db->query("SELECT COUNT(*) as count FROM features WHERE feature_name = 'integrations_marketplace'");
    $row = $checkIntegrationsFeature->fetch_assoc();
    
    if ($row['count'] == 0) {
        // Add integrations_marketplace feature
        $db->query("INSERT INTO features (feature_name, enabled, added_version, description) 
                    VALUES ('integrations_marketplace', 1, '$latestVersion', 'Connect with popular platforms and services to extend functionality')");
        $dbUpdates[] = "Added Integrations Marketplace feature to features table";
    }
}
